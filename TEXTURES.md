...
## チップスロットの枠（インベントリ画面の背景）

インベントリ画面（E キー）の右側 3 列 × 3 行（9 マス）が、チップスロットです。**枠の絵は、画面の背景そのものに描いてあります**（チップを置いても、枠は消えません）。空きマスにゲームが置いている目印のアイテムは、**透明**（見えない）で、ツールチップも出ません。

| ファイル | 内容 |
|---|---|
| `assets/minecraft/textures/gui/container/inventory.png` | インベントリ画面の背景（256×256）。右側 3×3 の枠を描いてある |
| `assets/minecraft/textures/gui/container/crafting_table.png` | 作業台の画面の背景。同じ位置に、同じ枠を描いてある |
| `assets/minecraft/textures/item/chip_slot.png` | **枠の絵の元**（16×16。1 マスぶん）。9 マスとも、この絵を貼る |

- 枠の位置は、背景の中の `x = 116 / 134 / 152`、`y = 84 / 102 / 120` から始まる 16×16 の 9 か所です（マスの内側。外の縁は、元のままです）。
- **枠の絵を描き直すとき**: `chip_slot.png` を描き直して、`python3 tools/make_gui.py <Minecraft のクライアント jar>` を実行すると、上の 2 つの背景が作り直されます（jar は `versions/<版>/<版>.jar`。Pillow が必要）。背景を直接描き直してもかまいません。
- 背景の元は、Minecraft の素材です（jar から取り出して、枠の部分だけを書き換えています）。
- チェストなどの、ほかの画面の背景は、書き換えていません（そこでは、チップスロットの位置は、普通の空きマスに見えます）。
- 目印のアイテムを見えなくしているので、`chip_slot` のアイテム定義（`items/chip_slot.json`）は、今は使っていません。
...
| id | 備考 |
|---|---|
| `transparent` | 透明な絵。チップスロットの、空きマスの目印に使っている（見えなくするため。書き換えないでください） |
